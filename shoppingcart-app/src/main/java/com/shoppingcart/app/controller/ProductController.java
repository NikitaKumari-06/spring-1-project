...
public class ProductController {
	...
	//get Product handler by product name and product scale
	@GetMapping("products/name/{productName}/{productScale}")
	public ResponseEntity<List<Product>> getProductByNameAndScale(@PathVariable("productName") String productName, @PathVariable("productScale") String productScale) throws ProductNotFoundException{
		List<Product> product = productService.getProductByNameAndScale(productName, productScale);
		
		if(product == null) {
			throw new ProductNotFoundException("Product not exists with productName : " + productName + "and productScale : " + productScale);
		}
		return ResponseEntity.of(Optional.of(product));
	}
	...
	//sorting implementation by quantity in stock
